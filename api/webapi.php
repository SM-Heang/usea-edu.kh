	...
	switch ($_GET['action']) {
        ...
        case 'events_en':
            $query = "WITH RankedEvents AS (
                ...
                ORDER BY usea_event_status.event_status_en DESC -- Add ORDER BY clause here
            )
            SELECT
                id,
                event_name,
        ...
        case 'events_kh':
            ...
            $query = "WITH RankedEvents AS (
                SELECT
                    event_id as id,
                    usea_event_status.event_status_kh as event_name,
                    event_cover as image,
                    event_title_kh as title,
                    event_description_kh as description,
                    event_date,
                    ROW_NUMBER() OVER (PARTITION BY usea_event_status.event_status_kh ORDER BY event_id DESC) AS RowNum
                FROM
                    usea_events
                JOIN
                    usea_event_status ON usea_events.event_status = usea_event_status.status_id
                ORDER BY usea_event_status.event_status_kh DESC -- Add ORDER BY clause here
            )
            SELECT
                id,
                event_name,
            ...
            FROM
                RankedEvents
            WHERE
                RowNum <= 10 ORDER BY event_name ASC";;
