...
if(isset($_GET['action'])){
    switch ($_GET['action']) {
        ...
        case 'study_program_en':
            ...
            usea_major.major_info_en as major_info, usea_study_year.study_year_en as year_name, usea_semester.semester_name_en as semester_name, usea_subject.subject_name_en as subject_name, total_credit, credit
            FROM usea_study_plan, usea_faculty, usea_major, usea_degree, usea_study_year, usea_semester, usea_subject , usea_faculty_icon
            WHERE usea_study_plan.fac_name = usea_faculty.fac_id 
            AND usea_study_plan.major_name = usea_major.major_id 
            ...
            while ($stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                if ($total_credit_key === false) {
                    ...
                    $total_credit_key = count($data['program_data'][$faculty_key]['faculty_data']['major_name'][$major_key]['major_data'][$degree_key]['degree_detail']['degree_data']) - 1;
                }
                 $data['program_data'][$faculty_key]['faculty_data']['major_name'][$major_key]['major_data'][$degree_key]['degree_detail']['degree_data'][$year_key]['year_data'][] = $subject_data;
